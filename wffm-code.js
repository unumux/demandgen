(function() {
    ...
    if (thisForm === null)
        //if the form is not found, bail out!
        return;
    ...
    const eeNumber =
        getFormFieldsContainer(thisForm, 'label', 'company size') ||
    ...
    function getFormFieldsContainer(form, element, data) {
        let items = form.querySelectorAll(element),
            parent = null;
    ...
    function handleEENumber(state) {
        ...
        if (state === 'hide') {
            ...
            if (eeNumType === 'text') eeNumInput.value = 0;
            else eeNumInput.value = '2 - 9';
        } else {
            eeNumInput.value = '';
